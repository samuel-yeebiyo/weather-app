...
const MainScreen = ({save, favs, current, weather, now}) => {
    ...
    return (
        <div className="Main">
            <div className="current">
                ...
                {Object.keys(now.data[0].weather).length !== 0 ?
                    <img className="weather-icon" src={process.env.PUBLIC_URL+ `/icons/${now.data[0].weather.icon}.png`}/> :
                    <img className="weather-icon" src=""/>
                }
                <p className="weather-description">{now.data[0].weather.description || "- -"}</p>
                <div className="current-temp">
                    <span className="cur-num">{now.data[0].temp || "- -"}</span><span className="cur-degree"></span>
                </div>
                <div className="apparent-temp">
                    <span className="app-num">{(now.data[0].app_temp) || "- -"}</span><span className="app-degree"></span>
                </div>
                <div className="meta">
                    ...
                    <div className="meta-item pres">
                        <img src={Bar}/>
                        <p>{now.data[0].pres || "- -"}  mb</p>
                    </div>
                    <div className="meta-item spd">
                        <img src={WinSpeed}/>
                        <p>{now.data[0].wind_spd || "- -"}  m/s</p>
                    </div>
                </div>
            </div>
